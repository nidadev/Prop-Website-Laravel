...
class ApiController extends BaseController
{
    ...
    public function GetPriceReport(Request $request)
    {
        ...
        try {
            ...
            $maxcount = $data['MaxResultsCount'];
            $acr_range = $acr1 . '-' . $acr2;
            $p_id = $data['LitePropertyList'][0]['PropertyId'];
            //$poperty_id = $data['Reports']['0']['PropertyId'];
            ...
            $mainval = $maxcount * 0.01;
            //now get property id sal eprice 
            return view('priceland', compact('maxcount', 'mainval', 'acr_range', 'data', 'sale_price'));
        } catch (\Exception $e) {
            //throw new HttpException(500, $e->getMessage());
            $response = $e->getResponse();
            $msg = json_decode($response->getBody()->getContents(), true);
